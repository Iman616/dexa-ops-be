...
->leftJoin('proforma_invoices as pi', 'pi.proforma_id', '=', 'i.proforma_invoice_id')
        ...
        ->select(
            ...
            'i.subtotal', 'i.tax_percentage', 'i.tax_amount',
            'i.discount_amount', 'i.total_amount', 'i.payment_status',
            'i.po_id', 'i.proforma_invoice_id',
            'c.customer_name', 'co.company_code',
            'at.type_code as unit_code', 'at.type_name as unit_name',
            'pi.proforma_date as tanggal_pi', 'po.po_number',
        )
   ...
   public function getSalesReport(Request $request)
{
    ...
    public function getSupplierPaymentReport(Request $request)
    {
        try {
            ...
            if (!$this->hasMenuAccess($roleId, 'supplierpayments') &&
                !$this->hasMenuAccess($roleId, 'financialreport')) {
                return response()->json(['success' => false, 'message' => 'Akses ditolak'], 403);
            }
            ...
            $rows = DB::table('supplier_payments as sp')
                ...
                ->leftJoin('supplier_purchase_orders as spo',
                                                       'spo.supplier_po_id',     '=', 'si.supplier_po_id')
                ->leftJoin('companies as co',          'co.company_id',          '=', 'spo.company_id')
                ->when($roleId !== 1 && $companyId, fn($q) =>
